        ...
        for key, value in options.items():
            try:
                ...
                db.execute("""INSERT INTO answers 
                    (id, questionId, optionLabel, optionText, isCorrect)
                    VALUES (?, ?, ?, ?, ?)""",
                    generate_user_id(value), id, key, value, isCorrect)
            except Exception as e:
                return "failure: inserting answers failed", 500
        
        if hasImage == 1:
            try:
                image_url = url_for('static', filename=f'uploads/{id}.jpg')  # usable in <img src="">
                dc.execute("""INSERT INTO questionImages (id, questionId, imagePath, altText)
